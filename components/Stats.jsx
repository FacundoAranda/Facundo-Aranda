...
const stats = [
  {
    num: 4,
    text: "Recomendaciones recibidas",
  },
  {
    num: 8,
    text: "Soft-Skills trabajadas",
  },
  {
    num: 25,
        ...
        <div className="flex flex-wrap gap-6 max-w-[80vw] mx-auto xl:max-w-none">
          {stats.map((item, index) => {
            return <div className="flex-1 flex gap-4 items-center justify-center xl:justify-start" key={index}>
              ...
              <p className={`${item.text.length < 15 ? "max-w-[100px]": "max-w-[150px]"} leadding-snug text-white/80` }>{item.text}</p>
            </div>
          })}
        </div>
      </div>
